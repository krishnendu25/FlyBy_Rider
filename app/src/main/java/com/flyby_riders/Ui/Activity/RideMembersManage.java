...
public class RideMembersManage extends BaseActivity implements onClick {
                ...
                if (response.isSuccessful()) {
                    try {
                        ...
                        if (jsonObject.getString("success").equalsIgnoreCase("1")) {
                            ...
                            if (new Prefe(getApplicationContext()).getAccountPlanStatus().equalsIgnoreCase(PREMIUM))
                            ...
                            ride_members_adapter = new Ride_Members_Adapter(RideMembersManage.this,Member_List);
        ...
        switch (view.getId()) {
            ...
            case R.id.Add_member_btn:
                    if (new Prefe(getApplicationContext()).getAccountPlanStatus().equalsIgnoreCase(PREMIUM))
                    {
                        ...
                        {
                            Intent intent = new Intent(getApplicationContext(), InviteNewMember.class);
    ...
    {
        show_ProgressDialog();
        Call<ResponseBody> requestCall = retrofitCallback.add_memeber_toride(new Prefe(this).getUserID(),my_ride_id,new_member,Ride_Name);
        requestCall.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                hide_ProgressDialog();
