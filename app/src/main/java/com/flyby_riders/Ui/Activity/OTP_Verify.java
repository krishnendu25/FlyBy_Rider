...
public class OTP_Verify extends BaseActivity implements IJSONParseListener {
                ...
                if (response.isSuccessful()) {
                    try {
                        ...
                        if (jsonObject.getString("success").equalsIgnoreCase("1")) {
                            ...
                            new Session(getApplicationContext()).set_LOGIN_USER_ID(jsonObject.getString("uid"));
                            new Session(getApplicationContext()).set_LOGIN_PHONE_NO(Phone_Number);
                            new Session(getApplicationContext()).set_IsLogin("true");
                            Intent intent = new Intent(getApplicationContext(), DashBoard.class);
                            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                            startActivity(intent);
                            finish();
                        } else {
                            new Session(getApplicationContext()).set_IsLogin("false");
    ...
    private void Send_OTP_AGAIN() {
        show_ProgressDialog();
        Call<ResponseBody> requestCall = retrofitCallback.RiderLogin(Phone_Number);
