...
public class Create_Group_Ride extends BaseActivity implements OnMapReadyCallback, CompoundButton.OnCheckedChangeListener {
    ...
    private void hit_Ride_api(boolean Update_Name, boolean Start_ride, boolean End_ride, boolean schedule, boolean My_Ride, boolean create_ride) {
        ...
        if (Start_ride) {
            ...
            show_ProgressDialog();
            requestCall = retrofitCallback.END_RIDE(My_Ride_ID, new Session(Create_Group_Ride.this).get_LOGIN_USER_ID(), GET_timeStamp(), Constant.getCompleteAddressString(Create_Group_Ride.this, Latitude_Start, Longitude_Start), String.valueOf(Latitude_Start), String.valueOf(Longitude_Start), "0");
        } else if (schedule) {
            show_ProgressDialog();
            ...
            LocalBroadcastManager.getInstance(getApplicationContext()).registerReceiver(
                    new BroadcastReceiver() {
                        ...
                        public void onReceive(Context context, Intent intent) {
                            ...
                            if (RIDE_STATUS.equalsIgnoreCase(RIDE_STARTED)) {
                                testAdapter.INSERT_REALTIMELOCATION(My_Ride_ID, new Session(getApplicationContext()).get_LOGIN_USER_ID(), String.valueOf(currentLatitude),
                                        String.valueOf(currentLongitude), GET_timeStamp());
